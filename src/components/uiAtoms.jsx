...
export const Field = ({ label, id, value, onChange, prefix, suffix, hint, readOnly, labelColor }) => {
  const computedLabelColor = labelColor || C.muted;
  return (
    <div style={{ marginBottom: "11px", color: computedLabelColor }}>
      <label style={{ display: "flex", justifyContent: "space-between", fontSize: "15px", color: computedLabelColor, letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: "4px" }}>
        <span>{label}</span>
        {hint && <span style={{ color: C.text }}>{hint}</span>}
      </label>
      <div style={{ display: "flex", alignItems: "center", background: C.bg, border: `1px solid ${C.border}`, borderRadius: "5px", overflow: "hidden" }}>
        ...
        <input
          ...
          readOnly={readOnly}
          style={{ flex: 1, background: "transparent", border: "none", outline: "none", color: C.text, padding: "5px 8px", fontSize: "17px", fontFamily: "monospace", minWidth: 0, borderRadius: "0" }}
        />
        {suffix && <span style={{ padding: "5px 8px", color: C.text, fontSize: "16px", borderLeft: `1px solid ${C.border}`, background: C.bgHov, whiteSpace: "nowrap" }}>{suffix}</span>}
      </div>
    </div>
  );
};
...
export const Card = ({ children, style = {}, variant }) => {
  ...
  return (
    <div style={{
      background: isPurple ? "#F3F0FF" : C.bgCard,
